 ...
 * ========================= 
 *
 * Integrasi AI Mentor diubah dari OpenAI ke Gemini API.
 *
 * Catatan Penting:
 * Kunci API disimpan di frontend untuk mempermudah, namun ini tidak aman
 * untuk aplikasi yang disebarkan ke publik.
 *
 */
...
  userName: '',
  
  // === BAGIAN PENTING UNTUK INTEGRASI GEMINI ===
  // GANTI 'KUNCI_API_GEMINI_ANDA_DI_SINI' DENGAN KUNCI ASLI DARI AI Studio.
...
/**
 * Menghubungi Gemini API untuk mendapatkan respons
...
async function getGeminiChatResponse(userMessage) {
    // 1. Cek Kunci API dan Ketersediaan Library (dari CDN di HTML)
    if (!appState.geminiApiKey || !window.google || !window.google.genai) {
        // Pesan error jika library atau kunci belum dimuat
        return "ERROR: Library Gemini atau API Key tidak terkonfigurasi. Pastikan Anda sudah menambahkan CDN di HTML dan kunci di script.js.";
    }

    const currentTopicTitle = currentTopic().title;
    
    // Inisialisasi klien dengan kunci API
    const ai = new google.genai.Client({ apiKey: appState.geminiApiKey });

    // 2. Tentukan Konteks (System Prompt)
    const systemInstruction = `Kamu adalah 'Mentor BelajarBareng' yang positif, sabar, dan suportif. Balas semua pertanyaan dalam Bahasa Indonesia. Topik yang sedang dipelajari saat ini adalah **${currentTopicTitle}** (${appState.currentSubject}). Jawablah sebagai mentor yang suportif, fokus pada konsep pelajaran, dan dorong pengguna untuk terus mencoba.`;

    try {
        // Panggilan API menggunakan SDK
        const response = await ai.models.generateContent({
            model: "gemini-2.5-flash", // Model Google yang cepat, gratis, dan efisien
        ...
        console.error("Kesalahan API/Koneksi Gemini:", e);
        // Menangani masalah otentikasi/kuota
        if (e.message && (e.message.includes('API key not valid') || e.message.includes('400') || e.message.includes('403'))) {
             return `Maaf, terjadi kesalahan API (Kemungkinan kunci salah, kuota habis, atau masalah jaringan). Detail: ${e.message}`;
        }
        return `Terjadi kesalahan koneksi tak terduga. Periksa jaringan atau konsol browser.`;
    }
}
...
  
  // Logika perintah khusus (Ringkasan/Ulang Soal)
...
  }
  
  // --- INTEGRASI GEMINI DIMULAI DI SINI ---
  // Cek apakah API Key sudah terisi dan library tersedia
  if(appState.geminiApiKey && appState.geminiApiKey.length > 10 && window.google && window.google.genai){
    postMentorMessage('Menghubungkan ke layanan AI, mohon tunggu...', 'ai');
    
    // Panggil fungsi asinkron Gemini
    getGeminiChatResponse(v) 
        .then(aiResponse => {
            postMentorMessage(aiResponse, 'ai');
        })
        .catch(e => {
            postMentorMessage('Gagal mendapatkan respons AI karena masalah teknis.', 'ai');
            console.error("Kesalahan saat memanggil Gemini:", e);
        });
    return;
  } else {
    postMentorMessage('Maaf, Kunci API Gemini belum diatur atau library tidak dimuat. Coba "ringkasan" atau "ulang soal".', 'ai');
  }
});
